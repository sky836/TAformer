...
class FullAttention(nn.Module):
    def __init__(self, n_heads, d_feature, d_keys=None, d_values=None, dropout=0.1):
...
class TimeAttentionLayer(nn.Module):
    def __init__(self, n_heads, d_model, d_keys=None, d_values=None, dropout=0.1):
        super(TimeAttentionLayer, self).__init__()
        self.d_keys = d_keys
        self.d_values = d_values

        self.time_attn = FullAttention(n_heads=n_heads, d_feature=d_model, d_keys=d_keys, d_values=d_values,
                                       dropout=0.1)
        self.target_attn = FullAttention(n_heads=n_heads, d_feature=d_model, d_keys=d_keys, d_values=d_values,
                                         dropout=0.1)

        self.time_out_projection = nn.Linear(d_values * n_heads, d_model)
        self.target_out_projection = nn.Linear(d_values * n_heads, d_model)

        self.n_heads = n_heads
        self.merge_projection = nn.Linear(2, 1)

        self.dropout = nn.Dropout(dropout)
        self.Timelayer_norm = nn.LayerNorm(d_model)
        self.Targetlayer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, time_features_Q, target_features_Q, time_features_K, target_features_K, attn_mask=None):
        ...
        time_attn_value = torch.matmul(time_attn_score, time_values).transpose(2, 3).reshape(batch_size, n_nodes, q_len,
                                                                                             -1)
        ...
        # ==================================2 target process============================================
        target_attn_score, target_values = self.target_attn(target_features_Q, target_features_K, target_features_K,
                                                            attn_mask)
        ...
        # 加权后再进行一次softmax
        merge_attn_score = self.dropout(torch.softmax(merge_attn_score, dim=-1))
...
class DecoderTimeAttention(nn.Module):
    def __init__(self, n_heads, d_model, d_keys=None, d_values=None, dropout=0.1):
        ...
        self.time_attn = FullAttention(n_heads=n_heads, d_feature=d_model, d_keys=d_keys, d_values=d_values,
                                       dropout=0.1)
        self.cross_value_projection = nn.Linear(d_model, d_values * n_heads)
    ...
    def forward(self, time_features_Q, time_features_K, target_features_K=None):
        ...
        time_value = self.out_projection(time_value)
        time_value = self.Timelayer_norm(time_value + time_features_Q)
        if target_features_K is not None:
            target_features_K = self.cross_value_projection(target_features_K).view(batch_size, n_nodes, k_len,
                                                                                    self.n_heads, -1).transpose(2, 3)
            cross_target = torch.matmul(attn_score, target_features_K).transpose(2, 3).reshape(batch_size, n_nodes,
                                                                                               q_len, -1)
            cross_target = self.out_projection(cross_target)
