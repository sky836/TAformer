...
class STEP(nn.Module):
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        hidden_states = None
        adj, adj_mx = load_adj(r"D:\博士阶段科研\文献阅读与汇报\时空数据挖掘\KDD\STEP-github\datasets\METR-LA\adj_mx.pkl", 'doubletransition')
        sampled_adj=adj_mx
        ...
        # return y_hat.unsqueeze(-1), gsl_coefficient

        return y_hat.unsqueeze(-1), None, None, gsl_coefficient
